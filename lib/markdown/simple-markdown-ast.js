    ...
    function generate(depth, comment) {
        ...
        function paramList(params) {
            return u('list', {ordered: false}, params.map(function (param) {
                var description = param.description ? [u("text", param.description || "")] : [];
                ...
                var typeNodes = param.type ? [
                    u('text', ':'),
                    u('inlineCode', stringifyType(param.type))
                ] : [];
                var inParagraph = [
                    u('strong', [
                        u('text', param.name)
        ...
        function propertyList(properties) {
            return u('list', {ordered: false},
                properties.map(function (property) {
                    var descriptions = property.description ? [u("text", property.description || "")] : [];
                    return u('listItem', [
                        u('paragraph', [
                            u('strong', [
                                u('text', property.name)
                            ]),
                            u('text', ':'),
                            u('inlineCode', stringifyType(property.type)),
                            u('text', ' - ')
                        ].concat(descriptions)
                            .filter(isNotUndefined)),
        ...
        function returnsSection(comment) {
            return !!comment.returns && comment.returns.map(function (returns) {
                    var description = returns.description ? u("text", returns.description || "") : [];
                    return u('paragraph', [
                        u('strong', [
                            u('text', 'Returns')
                        ]),
                        u('text', ':'),
                        u('inlineCode', stringifyType(returns.type)),
                        u('text', ' - ')
                    ].concat(description).filter(isNotUndefined));
                });
        }
