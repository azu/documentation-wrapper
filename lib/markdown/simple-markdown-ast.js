...
function commentsToAST(comments, opts, callback) {
    ...
    function generate(depth, comment) {
        function descriptionAsMarkdown(description, prefix) {
            ...
            var descriptionAST = remark.parse(description);
            if (prefix) {
                return [u('text', prefix)].concat(descriptionAST.children)
            }
            return descriptionAST.children;
        }
        ...
        function $paramList(params) {
            var $paramList = params.filter(contain$destructuringParam);
            var normalParams = params.filter(param => {
             ...
             */
            const u$paramLists = $paramList.map(function ($param) {
                // `$0`: **Object**
                // param.name : param.type.name
                var listTitle = $param.name;
                return u("listItem", [
                    u("inlineCode", listTitle + ": " + $param.type.name),
                    // ul
                    paramList($param.properties)
                ]);
            });
            return u("list", {ordered: false}, u$paramLists);
        }
        ...
        function paramPairString(params) {
            ...
            // When contain $, only use $ params
            if (params.some(contain$destructuringParam)) {
                var destructuringParams = params.filter(contain$destructuringParam);
                return destructuringParams.sort((a, b) => {
